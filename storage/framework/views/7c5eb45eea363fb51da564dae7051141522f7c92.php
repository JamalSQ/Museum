...
unset($__errorArgs, $__bag); ?>
      ...
      <div class="form-group <?php echo e(($product->cat_id == 1) ? '' : 'd-none'); ?>" id="mainfile_div">
        <label for="price" class="col-form-label">Zip File <span class="text-danger">*</span></label>
        <input type="file" name="mainfile" id="mainfile" class="form-control" accept=".zip,.rar">
        <?php if($product->mainfile): ?>
        <span><a href="<?php echo e(asset($product->mainfile)); ?>" target="_blank">Click To Dowload</a></span>
        <?php endif; ?>
        <?php $__errorArgs = ['mainfile'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?>
...
<script>
  ...
  $('#cat_id').change(function() {
    ...
    if (cat_id != null) {
      if (cat_id == 1) {
        $('#mainfile_div').removeClass('d-none');
      } else {
        $('#mainfile_div').addClass('d-none');
      }
      // ajax call
      $.ajax({
...
<?php $__env->stopPush(); ?>
<?php echo $__env->make('backend.layouts.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\Museum\resources\views/backend/product/edit.blade.php ENDPATH**/ ?>